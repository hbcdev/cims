...
FROM Q_memb ;	
INTO TABLE (gcFundCode+"_PPP_member")	
*
SELECT plan, ;
...
INTO TABLE (gcFundCode+"_PPP_Claim")
**
...
SELECT Claim.notify_no, Claim.notify_date, Claim.claim_date, IIF(Claim.service_type = "IPD", "IPD", "OPD") AS serv_type, ;
	...
	Claim.abenfpaid AS exgratia, ;
	Claim.scharge AS charge, ;		
	Claim.sbenfpaid+Claim.over_respond AS paid, ;
	IIF(INLIST(result, "W5", "P5"), "F", LEFT(result,1)) AS status, ;		
...
INTO TABLE (gcFundCode+"_PPP_Claim_Month")
***
...
SELECT A.fundcode, A.notify_no, A.claim_date,  A.serv_type,  A.policy_no, A.plan, A.plan_type, A.return_dat, ;
  ...
  Claim_line.scharge AS charge, ;
  Claim_line.spaid+Claim_line.dpaid AS paid, ;
  Claim_line.apaid AS exgratia, A.result, A.status ;  
FROM (gcFundCode+"_PPP_Claim_Month") A INNER JOIN cims!claim_line ;
	ON  A.notify_no = Claim_line.notify_no ;
WHERE A.serv_type = "IPD" ;
	AND A.status $ "PF" ;	
INTO TABLE (gcFundCode+"_PPP_Claimline")
*
...
FROM (gcFundCode+"_PPP_Claim_Month") ;
GROUP BY 1 ;
WHERE (result = "P1" OR result like "D%") ;
	AND EMPTY(unclean) ;
INTO TABLE (gcFundCode+"_PPP_Aging")
*
**********************************
...
FROM _SumType RIGHT JOIN _SumMember ;
	ON _SumType.plan = _SumMember.plan ;
ORDER BY 1 ;		
INTO TABLE (gcFundCode+"_PPP_PlanbyService_Year")
*
**********************************
...
FROM (gcFundCode+"_PPP_Claimline") ;
WHERE cat_code # "OTHER" ;
GROUP BY plan, cat_code, benefit ;
ORDER BY plan ;
INTO TABLE (gcFundCode+"_PPP_PlanbyCategory_Month")
*
...
*
CREATE TABLE (lcRollingFile) FREE (months C(6), nom Y, eqal Y, ep Y, p1paid Y, p5paid Y, wpaid Y)
*
...
DO WHILE ldEndDate < gdEndDate
	...
	**************************************************************************************
	...
	Claim.admis_date, Claim.sbenfpaid AS paid, ;
	...
	*
	...
	AND Claim.result = "P5" ;	
	AND claim.return_date BETWEEN ldStartDate AND ldEndDate ;			
	INTO CURSOR curClaimP5	
	*	
	SELECT Claim.notify_no, Claim.notify_date, Claim.claim_date, IIF(Claim.service_type = "IPD", "IPD", "OPD") AS service, ;
	Claim.admis_date, IIF(Claim.sbenfpaid = 0, Claim.fbenfpaid, Claim.sbenfpaid) AS paid, ;
	IIF(INLIST(result, "W5", "P5"), "F", LEFT(result,1)) AS status, Claim.result, Claim.return_date ;
	FROM cims!claim ;
	WHERE Claim.fundcode = gcFundCode ;
	AND Claim.policy_no = lcPolicyNo ;	
	AND Claim.claim_with = "T" ;
	AND Claim.result like "W%" ;	
	INTO CURSOR curClaimW
	*		
	SELECT curClaimP1
	SUM paid TO m.p1paid
	*
	SELECT curClaimP5
	SUM paid TO m.p5paid
	*
...
lcRollingFile = gcFundCode+"_PPP_ClaimRolling"
*
