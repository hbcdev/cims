...
lnRollDays = (365.25/12)*gnRolling
...
DO q_Agentbycause_month
DO q_Agentbycause_year
DO query_rolling
*************************
SET TALK OFF 
...
SELECT Member.tpacode, Member.policy_no, Member.product AS plan, IIF(Member.customer_type = "P", "PA", "HS") AS plan_type, ;
	Member.effective, Member.expiry, Member.premium, Member.premium/lnRollDays AS prem_day, ;
	Member.branch_code, Member.agent_name, Member.agent ;
 FROM cims!Member ;
 WHERE Member.tpacode = gcFundCode ;
	AND Member.expiry >= gtStartDate ;
	AND INLIST(customer_type, "A", "P") ;
 INTO CURSOR Q_member
*
SELECT tpacode, policy_no, plan, effective, expiry, premium, prem_day, branch_code, ALLTRIM(agent_name)+"/"+ALLTRIM(agent) AS agent, ;	
...
INTO CURSOR Q_memb
*
SELECT tpacode, policy_no, plan, effective, expiry, premium, prem_day, branch_code, agent, ;	
...
SELECT Claim.notify_no, Claim.notify_date, IIF(Claim.service_type = "IPD", "IPD", "OPD") AS serv_type, Claim.cause_type, ;
	...
	IIF(EMPTY(Claim.fax_by), scharge-sdiscount, fcharge-fdiscount) AS charge, ;
	IIF(EMPTY(Claim.fax_by), sbenfpaid, fbenfpaid) AS paid, ;	
	IIF(EMPTY(Claim.fax_by), snopaid, fnopaid) AS nopaid, ;
	IIF(EMPTY(Claim.fax_by), sremain, fremain) AS overpaid, ;	
	IIF(INLIST(result, "P62", "W5"), "F", LEFT(result,1)) AS status, Claim.result, Claim.return_date, Claim.unclean, Claim.unclean_note, Claim.branch, ;
	ALLTRIM(Claim.agent)+"/"+ALLTRIM(Claim.agent_code) AS agent,  ;
	...
	((IIF(EMPTY(paid_date), return_date, paid_date) - TTOD(notify_date)) - holidays(TTOD(notify_date), IIF(EMPTY(paid_date), return_date, paid_date)))-1 AS workdays ;	
FROM  cims!claim ;
WHERE Claim.fundcode = gcFundCode ;
	AND INLIST(Claim.claim_with, "A", "P") ;	
	AND ((Claim.admis_date BETWEEN gtStartDate AND gtEndDate) OR (Claim.notify_date BETWEEN gtStartDate AND gtEndDate)) ;
...
SELECT Claim.notify_no, Claim.notify_date, IIF(Claim.service_type = "IPD", "IPD", "OPD") AS serv_type, Claim.cause_type, ;
	...
	Claim.admis_date, Claim.fcharge, Claim.fbenfpaid, Claim.scharge, Claim.sbenfpaid, Claim.sremain, Claim.snopaid, ;
	IIF(EMPTY(Claim.fax_by), abenfpaid, exgratia) AS exgratia, ;
	IIF(EMPTY(Claim.fax_by), scharge-sdiscount, fcharge-fdiscount) AS charge, ;
	IIF(EMPTY(Claim.fax_by), sbenfpaid, fbenfpaid) AS paid, ;	
	IIF(EMPTY(Claim.fax_by), snopaid, fnopaid) AS nopaid, ;
...
******************************************************************
PROCEDURE Q_AgentbyCause_Month
*
...
*
SELECT _sumbyprov.agent, _Sumbyprov.nom_month, _SumbyProv.eqa_month, _SumbyProv.ep_month, ;
...
FROM _provbycause RIGHT JOIN _SumbyProv ;
	ON _provbycause.agent = _SumbyProv.agent ;
ORDER BY 1 ;
INTO CURSOR _SumProvbyCause
*
SELECT agent, nom_month, eqa_month, ep_month, ;
...
*********************************************************************
PROCEDURE Q_AgentbyCause_Year
*
SELECT agent, ; 
	SUM(IIF(!INLIST(cause_type, "MC", "PMC", "PMR", "PMW", "PMA", "PMS", "PMK", "PWA", "MA") AND status = "P", 1, 0)) AS me_p_noc, ;
	SUM(IIF(!INLIST(cause_type, "MC", "PMC", "PMR", "PMW", "PMA", "PMS", "PMK", "PWA", "MA") AND status = "P", paid, 0)) AS me_p_paid, ;
	SUM(IIF(cause_type = "MC" AND status = "P", 1, 0)) AS mc_p_noc, ;
	SUM(IIF(cause_type = "MC" AND status = "P", paid, 0)) AS mc_p_paid, ;
	SUM(IIF(cause_type = "PMC" AND status = "P", 1, 0)) AS pmc_p_noc, ;
	SUM(IIF(cause_type = "PMC" AND status = "P", paid, 0)) AS pmc_p_paid, ;	
...
INTO CURSOR _provbycause
*
SELECT _sumbyprov.agent, _Sumbyprov.nom_year, _SumbyProv.eqa_year, _SumbyProv.ep_year, ;
	_provbycause.me_p_noc, _provbycause.me_p_paid, _provbycause.mc_p_noc, _provbycause.mc_p_paid, _provbycause.pmc_p_noc, _provbycause.pmc_p_paid, ;	
	_provbycause.pmr_p_noc, _provbycause.pmr_p_paid, _provbycause.pmw_p_noc, _provbycause.pmw_p_paid, _provbycause.pma_p_noc, _provbycause.pma_p_paid, ;
	_provbycause.pms_p_noc, _provbycause.pms_p_paid, _provbycause.pmk_p_noc, _provbycause.pmk_p_paid, _provbycause.ma_p_noc, _provbycause.ma_p_paid, ;
	_provbycause.pwa_p_noc, pwa_p_paid, ;	
...
INTO CURSOR _SumProvbyCause
*
SELECT agent, nom_year, eqa_year, ep_year, ;
	IIF(ISNULL(me_p_noc), 0, me_p_noc) AS me_p_noc, me_p_paid, ;
	IIF(ISNULL(mc_p_noc), 0, mc_p_noc) AS mc_p_noc, mc_p_paid, ;
	IIF(ISNULL(pmc_p_noc), 0, pmc_p_noc) AS pmc_p_noc, pmc_p_paid, ;	
	IIF(ISNULL(pmr_p_noc), 0, pmr_p_noc) AS pmr_p_noc, pmr_p_paid, ;
...
starttime = Seconds()
oXtab = NewObject("FastXtab", "e:\hips_src\progs\FastXtab.prg")
