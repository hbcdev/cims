...
SELECT pv.pv_date, ;
	...
	pv.pv_no, pv.client_name, pv.paid_to, pv_notify.notify_no, ;
	pv.policy_no, pv_notify.amount, ROUND(Pv_notify.amount*(Pv.wt/100),2) AS tax, ;
	Pv_notify.amount-ROUND(Pv_notify.amount*(Pv.wt/100),2) AS paid, pv.chqdate, ;
	Pv.tr_acno, Pv.tr_accname, Pv.tr_bank, Pv.tr_branch, Pv.tr_date, ;
	Pv_notify.remarks, Pv.notes, Pv_notify.admit, ;
  IIF(LEN(ALLTRIM(pv.policy_no)) > 9, 1, 0) AS type ;
