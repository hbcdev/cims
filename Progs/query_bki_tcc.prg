...
********************
gcCaption = "BKI TCC Group Monthly Report"
gnAll = 1
gnCover = 1
gnData = 0
gnType = 1
...
SELECT Dependants.fundcode AS tpacode, Dependants.policy_no, Dependants.plan, ;
 ...
 WHERE Dependants.fundcode = gcFundCode ;
   AND INLIST(Dependants.policy_no, laPolNo[1], laPolNo[2], laPolNo[3], laPolNo[4], laPolNo[5], laPolNo[6], laPolNo[7]) ;
...
WHERE Claim.fundcode = gcFundCode ;
	AND INLIST(Claim.policy_no, laPolNo[1], laPolNo[2], laPolNo[3], laPolNo[4], laPolNo[5], laPolNo[6], laPolNo[7]) ;
	AND Claim.claim_with = "T" ;
	AND Claim.result # "C" ;	
	AND Claim.admis_date BETWEEN gtStartDate AND gtEndDate ;	
INTO TABLE (lcFileName+"_Claim")
**
*Query Current Month Report
*
SELECT Claim.notify_no, Claim.notify_date, Claim.claim_date, IIF(Claim.service_type = "IPD", "IPD", "OPD") AS serv_type, Claim.plan_id, ;
...
INTO TABLE (lcFileName+"_Claim_Month")
***
*Query Claim by service
SELECT A.fundcode, A.notify_no, A.claim_date,  A.serv_type,  A.policy_no, A.plan, A.plan_type, A.return_dat, A.plan_id, ;
  A.plan+Claim_line.cat_code AS plan_cat, Claim_line.cat_id, Claim_line.cat_code, Claim_line.description, ;
...
FROM (lcFileName+"_Claim_Month") A INNER JOIN cims!claim_line ;
	ON  A.notify_no = Claim_line.notify_no ;
WHERE A.serv_type = "IPD" ;
	AND A.status = "P" ;	
INTO CURSOR _claimline
*
SUSPEND 
SELECT A.fundcode, A.notify_no, A.claim_date,  A.serv_type,  A.policy_no, A.plan, A.plan_type, A.return_dat, A.plan_id, ;
  A.plan_cat, A.cat_id, A.cat_code, A.description, plan2cat.benefit, A.charge, A.paid, A.exgratia, A.result, A.status ;  
FROM _claimline A INNER JOIN cims!plan2cat ;
	ON  A.plan_id = plan2cat.plan_id AND A.cat_id = plan2cat.cat_id ;
INTO TABLE (lcFileName+"_Claimline")
*
...
FROM (lcFileName+"_Claim_Month") ;
GROUP BY 1 ;
WHERE (result = "P1" OR result like "D%") ;
	AND EMPTY(unclean) ;
INTO TABLE (lcFileName+"_Aging")
*
**********************************
...
ORDER BY 1 ;		
INTO TABLE (lcFileName+"_PlanbyService_Month")
...
FROM _SumType RIGHT JOIN _SumMember ;
	ON _SumType.plan = _SumMember.plan ;
ORDER BY 1 ;		
INTO TABLE (lcFileName+"_PlanbyService_Year")
*
**********************************
...
FROM (lcFileName+"_Claimline") ;
WHERE cat_code # "OTHER" ;
GROUP BY plan, cat_code, benefit ;
ORDER BY plan ;
INTO TABLE (lcFileName+"_PlanbyCategory_Month")
*
...
lcRollingFile = lcFileName+"_Rolling"
*
...
lcRollingFile = lcFileName+"_ClaimRolling"
*
