...
DO q_rolling
****************
= messageAlert("Generate Suscess", 1, 0, "Monthly Report", "Generate Report", "", "", "", "", "")
...
GROUP BY 1 ;
where len(alltrim(result)) = 2 ;
	and (result  = 'P1' or left(result,1)  = 'D') ;
	AND EMPTY(unclean) ;
...
DO WHILE ldEndDate < gdEndDate
	...
	*
	...
	SCATTER MEMVAR 
	***********************************************************************
	store 0 to m.notify, m.inotify, m.onotify, m.admit, m.iadmit, m.oadmit
	...
	*
	if _TALLY > 0
		m.notifys = laClaimN[3]
		m.inotify = laClaimN[4]
		m.onotify = laClaimN[5]
	endif
	*	
