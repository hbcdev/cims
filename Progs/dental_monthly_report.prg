...
PROCEDURE q_Member
*
WAIT WINDOW IIF(EMPTY(gcGroup), "", "Group Policy : "+gcGroup) NOWAIT 
IF EMPTY(gcGroup)
	lcWhere =  "Member.tpacode = gcFundCode AND Member.expiry >= gtStartDate AND customer_type = 'D' AND LEN(ALLTRIM(policy_no)) <= 17"
ELSE 
	lcGroup = ALLTRIM(gcGroup)+"%"	
	lcWhere =  "Member.tpacode = gcFundCode AND Member.expiry >= gtStartDate AND customer_type = 'D' AND policy_no LIKE lcGroup"
ENDIF 
*
...
*
IF EMPTY(gcGroup)
	lcWhere =  'Claim.fundcode = gcFundCode AND Claim.claim_with = "D" AND Claim.result # "C" AND Claim.admis_date BETWEEN gtStartDate AND gtEndDate AND LEN(ALLTRIM(policy_no)) <= 17' 
ELSE 	
	lcGroup = ALLTRIM(gcGroup)+"%"	
	lcWhere =  'Claim.fundcode = gcFundCode AND Claim.claim_with = "D" AND Claim.result # "C" AND Claim.admis_date BETWEEN gtStartDate AND gtEndDate AND policy_no LIKE lcGroup' 	
ENDIF 
*
SELECT Claim.notify_no, Claim.notify_date, Claim.service_type AS serv_type, Claim.cause_type, Claim.prov_name, ;
	...
	IIF(EMPTY(Claim.fax_by), Claim.scharge, Claim.fcharge) AS exgratia, ;	
...
FROM (lcFile+"_DEN_Claim") A INNER JOIN cims!claim_line ;
	ON  A.notify_no = Claim_line.notify_no ;	
	AND A.status = "P" ;	
WHERE claim_line.scharge <> 0 ;	
INTO TABLE (lcFile+"_DEN_Claimline_Year")
**
*Query Current Month Report
*
IF EMPTY(gcGroup)
	lcWhere =  'Claim.fundcode = gcFundCode AND Claim.claim_with = "D" AND Claim.result # "C" AND Claim.ref_date BETWEEN gtCurDate AND gtEndDate AND LEN(ALLTRIM(policy_no)) <= 17' 

ELSE 	
	lcWhere =  'Claim.fundcode = gcFundCode AND Claim.claim_with = "D" AND Claim.result # "C" AND Claim.ref_date BETWEEN gtCurDate AND gtEndDate AND policy_no LIKE gcGroup' 	
ENDIF 
...
FROM  cims!claim ;
WHERE &lcWhere ;
INTO TABLE (lcFile+"_DEN_Claim_Month")
***
*Query Claim by cause
SELECT A.fundcode, A.notify_no, A.notify_dat, A.cause_type, A.policy_no, A.plan, A.agent, ;
  ...
  IIF(EMPTY(A.fax_by), Claim_line.spaid, Claim_line.fpaid) AS paid, ;
  IIF(EMPTY(A.fax_by), Claim_line.apaid, Claim_line.exgratia) AS exgratia, A.result, A.status ;  
FROM (lcFile+"_DEN_Claim_Month") A INNER JOIN cims!claim_line ;
	ON  A.notify_no = Claim_line.notify_no ;
WHERE claim_line.scharge <> 0 ;
INTO TABLE (lcFile+"_DEN_Claimline_Month")
*
...
FROM (lcFile+"_DEN_Claim_Month") ;
GROUP BY 1 ;
WHERE INLIST(status, "D", "P") ;
	AND EMPTY(unclean) ;
	AND result # "P5" ;
INTO TABLE (lcFile+"_DEN_Aging")
*
**********************************************
...
GROUP BY 1 ;
ORDER BY 1 ;
INTO TABLE (lcFile+"_DEN_PlanbyItems_Month")
...
SELECT cause_type, ;
	...
	SUM(IIF(status = "W", 1, 0)) AS w_noc, ;
	SUM(IIF(status = "W", paid, 0)) AS w_paid ;
FROM (lcFile+"_DEN_Claim") ;
GROUP BY 1 ;
ORDER BY 1 ;
INTO TABLE (lcFile+"_DEN_PlanbyItems_Year")
*
**********************************************************
PROCEDURE q_rolling

lcRollingFile = lcFile+"_DEN_Rolling"
*
CREATE TABLE (lcRollingFile) FREE (months C(6), nom Y NULL, eqal Y NULL, ep Y NULL, notifys Y NULL, admit Y NULL)
*
...
DO WHILE ldEndDate < gdEndDate
	...
	***********************************************************************
	IF EMPTY(gcGroup)
		lcWhere1 =  'Claim.fundcode = gcFundCode AND Claim.claim_with = "D" AND Claim.result  = "P" AND Claim.ref_date BETWEEN ldStartDate AND ldEndDate AND LEN(ALLTRIM(policy_no)) <= 17' 
		lcWhere2 =  'Claim.fundcode = gcFundCode AND Claim.claim_with = "D" AND Claim.result  = "P" AND Claim.admis_date BETWEEN ldStartDate AND ldEndDate AND LEN(ALLTRIM(policy_no)) <= 17' 		
	ELSE 	
		lcGroup = ALLTRIM(gcGroup)+"%"	
		lcWhere1 =  'Claim.fundcode = gcFundCode AND Claim.claim_with = "D" AND Claim.result = "P" AND Claim.ref_date BETWEEN ldStartDate AND ldEndDate AND policy_no LIKE lcGroup' 
		lcWhere2 =  'Claim.fundcode = gcFundCode AND Claim.claim_with = "D" AND Claim.result = "P" AND Claim.admis_date BETWEEN ldStartDate AND ldEndDate AND policy_no LIKE lcGroup' 				
	ENDIF 
	*	
	SELECT notify_no, ref_date, IIF(EMPTY(Claim.fax_by), sbenfpaid, fbenfpaid) AS paid ;		
	FROM cims!claim ;
	WHERE &lcWhere1 ;
