...
****************************************
* query Data
lcTable = Query2Excel(tcFundCode, tcTable)
IF !USED("curquery")
	RETURN
ENDIF 
************************************************
...
DO WHILE !EOF()
	...
	DO WHILE pol_no = lcPolicyNo AND !EOF()
		...
		***********
		lnRow = 2
		DO WHILE pol_no = lcPolicyNo AND ALLTRIM(hosp_name) = lcType AND !EOF()
			FOR i = 1 TO FCOUNT()
				lcField = FIELD(i)
				lcValue = &lcField
				IF !EMPTY(lcValue)
