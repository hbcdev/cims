...
**************************************
lnSheet = 1
lcFilePath = ADDBS(tcSaveTo)+ALLTRIM(tcFundCode)+"\"+tcLotNo
...
*
IF LEFT(tcLotNo,1) = "C"
	lcFile = ADDBS(lcFilePath)+ALLTRIM(tcLotNo)+"_Return_"+STRTRAN(DTOC(return_date), "/", "")	
ELSE 
...
DO WHILE !EOF()
	...
	*
	...
	oSheet.Cells(lnRows, 2).Value = TRANSFORM(lnTotalClaim, "@Z 999,999")+" เคลม"
	lnRows = lnRows + 1	
	oSheet.Cells(lnRows, 1).Value = "ยอดจ่ายสะสมรวมทั้งสิ้น"
	oSheet.Cells(lnRows, 2).Value = TRANSFORM(lnTotalPaid, "@Z 9,999,999.99")
	lnRows = lnRows + 2
