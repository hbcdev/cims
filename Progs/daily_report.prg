...
ENDIF
**************************************
DIMENSION laComment[7,1], laTitle[40], laClaim[34], laFormat[40], laType[3,2]
...
SELECT IIF(Claim.result = "P5", "I", IIF(Claim.result = "W5", "F", IIF(Claim.result = "W6", "U", LEFT(Claim.result,1)))) AS clm_status, ;
	...
	Claim_line.total_fee AS fee, Claim.result, Claim.snote, Claim.note2ins, Claim.ref_date, Claim.return_date ;
				...
				FOR i = 1 TO lnMax && Fcount()
					...
					*******************************
					...
					IF !EMPTY(lcValue)
						...
						ENDIF
						IF !INLIST(lcField, "description", "charge", "discount", "paid", "overpaid", "nopaid", "exgratia")
