...
********************
gcCaption = "DVS Monthly Report(TTC)"
gnAll = 1
gnCover = 1
gnData = 0
gnType = 1
...
FROM Q_memb ;	
INTO TABLE (gcFundCode+"_TTC_member")	
*
SELECT plan, ;
...
INTO TABLE (gcFundCode+"_TTC_Claim")
**
...
INTO TABLE (gcFundCode+"_TTC_Claim_Month")
***
...
FROM (gcFundCode+"_TTC_Claim_Month") A INNER JOIN cims!claim_line ;
	ON  A.notify_no = Claim_line.notify_no ;
WHERE A.serv_type = "IPD" ;
	AND A.status = "P" ;	
INTO TABLE (gcFundCode+"_TTC_Claimline")
*
...
FROM (gcFundCode+"_TTC_Claim_Month") ;
GROUP BY 1 ;
WHERE status $ "DP" ;
	AND EMPTY(unclean) ;
	AND result # "P5" ;
INTO TABLE (gcFundCode+"_TTC_Aging")
*
**********************************
...
ORDER BY 1 ;		
INTO TABLE (gcFundCode+"_TTC_PlanbyService_Month")
...
FROM _SumType RIGHT JOIN _SumMember ;
	ON _SumType.plan = _SumMember.plan ;
ORDER BY 1 ;		
INTO TABLE (gcFundCode+"_TTC_PlanbyService_Year")
*
**********************************
...
FROM (gcFundCode+"_TTC_Claimline") ;
WHERE cat_code # "OTHER" ;
GROUP BY plan, cat_code, benefit ;
ORDER BY plan ;
INTO TABLE (gcFundCode+"_TTC_PlanbyCategory_Month")
*
...
lcRollingFile = gcFundCode+"_TTC_Rolling"
*
...
lcRollingFile = gcFundCode+"_TTC_ClaimRolling"
*
