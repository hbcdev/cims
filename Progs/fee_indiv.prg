...
gdEndDate = GOMONTH(gdStartDate,1)-1 &&DATE(YEAR(gdStartDate), MONTH(gdStartDate),IIF(INLIST(MONTH(gdStartDate), 1,3,5,7,8,10,12),31,IIF(MONTH(gdStartDate) = 2, 28,30)))
gnOption = 1
...
WHERE tpacode = gcFundCode ;
	AND adjrein BETWEEN gdStartDate AND gdEndDate ;
	AND (EMPTY(adjlapse) OR adjlapse BETWEEN gdStartDate AND gdEndDate) ;
	AND polstatus = "R" ;
ORDER BY 10, 5 ;	
INTO CURSOR aladata	
*********************************
