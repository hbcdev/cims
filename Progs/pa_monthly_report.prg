...
PROCEDURE q_Member
*
SELECT Member.tpacode, Member.policy_no, Member.product AS plan, IIF(Member.customer_type $ "PA", "PA", "HS") AS plan_type, ;
	Member.effective, Member.expiry, Member.premium, Member.premium/365.25 AS prem_day, ;
	Member.branch_code, ALLTRIM(agent_name)+"/"+ALLTRIM(agent) AS province ;
FROM cims!Member ;
WHERE Member.tpacode = gcFundCode ;
	AND Member.expiry >= gtStartDate ;
	AND !INLIST(Member.status, "C", "T") ;
...
PROCEDURE q_Claim
*
SELECT Claim.notify_no, Claim.notify_date, Claim.followup, Claim.claim_date, IIF(Claim.service_type = "IPD", "IPD", "OPD") AS serv_type, ;
	...
	Claim.plan, LEFT(Claim.plan,2) AS plan_type, Claim.admis_date, Claim.acc_date, Claim.fcharge, Claim.fbenfpaid, Claim.scharge, Claim.sbenfpaid, ;
	...
	IIF(INLIST(result, "W5", "P5", "P62"), "F", LEFT(result,1)) AS status, ;	
...
WHERE Claim.fundcode = gcFundCode ;
	...
	AND Claim.result # "C" ;	
	AND ((Claim.admis_date BETWEEN gtStartDate AND gtEndDate) OR (Claim.admis_date BETWEEN gtStartDate AND gtEndDate)) ;	
INTO TABLE (gcFundCode+"_PA_Claim")
**
*Query Current Month Report
*
SELECT Claim.notify_no, Claim.notify_date, Claim.followup, Claim.claim_date, IIF(Claim.service_type = "IPD", "IPD", "OPD") AS serv_type, Claim.cause_type, ;
	...
	Claim.admis_date, Claim.acc_date, Claim.fcharge, Claim.fbenfpaid, Claim.scharge, Claim.sbenfpaid, Claim.fax_by, ;
	IIF(EMPTY(Claim.fax_by), Claim.abenfpaid, Claim.exgratia) AS exgratia, ;
	IIF(EMPTY(Claim.fax_by), Claim.scharge, Claim.fcharge) AS charge, ;		
	IIF(EMPTY(Claim.fax_by), Claim.sbenfpaid+Claim.over_respond, Claim.fbenfpaid+Claim.deduc) AS paid, ;
	IIF(EMPTY(Claim.fax_by), Claim.sremain, Claim.fremain) AS remain, ;			
...
FROM _planbycause RIGHT JOIN _SumMember ;
	ON _planbycause.plan = _SumMember.plan ;
ORDER BY 1 ;
...
*
CREATE TABLE (lcRollingFile) FREE (months C(6), nom Y, eqal Y, ep Y, opd_amt1 I, opd_paid1 Y, ipd_amt1 I, ipd_paid1 Y, opd_amt2 I, opd_paid2 Y, ipd_amt2 I, ipd_paid2 Y, out_amt1 I, out_paid1 Y, d_amt1 I, dom I)
*
ldStartDate = DATE(YEAR(gdStartDate), MONTH(gdStartDate), DAY(gdStartDate))
lnDay = ICASE(MONTH(ldStartDate) = 2, IIF(MOD(YEAR(ldStartDate),4) = 0, 29, 28), INLIST(MONTH(ldStartDate), 1,3,5,7,8,10,12), 31, 30)
ldEndDate = DATE(YEAR(ldStartDate), MONTH(ldStartDate), lnDay)
