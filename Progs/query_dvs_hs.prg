...
SELECT Member.tpacode, Member.policy_no, Member.product AS plan, ;
	LEFT(Member.product,2) AS plan_type, Member.hb_limit, Member.effective, Member.expiry, Member.premium, ;
	VAL(cause1) AS prem_i, VAL(cause2) AS prem_o, year(Member.expiry) - year(Member.effective) AS yy , polstatus ;
 FROM cims!Member ;
 WHERE Member.tpacode = gcFundCode ;
   AND Member.expiry >= gtStartDate ;
   AND substr(member.policy_no,9,1) <> "R";
...
ENDIF 
*	
SELECT tpacode, policy_no, plan, plan_type, hb_limit, effective, expiry, yy, prem_i, prem_o, premium, polstatus, ;
	iif(premium = 0 or yy = 0, 0, premium/yy) AS prem_year, ;
	iif(premium = 0 or yy = 0, 0, premium/(365.25*yy)) AS prem_day, ;
...
FROM Q_memb ;	
INTO TABLE (gcFundCode+"_H_member")	
*
SELECT plan, ;
...
SELECT Claim.notify_no, Claim.notify_date, IIF(Claim.service_type = "OPD", "OPD", "IPD") AS serv_type, ;
	...
	Claim.admis_date, Claim.fcharge, Claim.fbenfpaid, Claim.scharge, Claim.sbenfpaid, Claim.inv_page, ;
...
WHERE Claim.fundcode = gcFundCode ;
	AND substr(claim.policy_no,9,1) = "R";
	AND Claim.result # "C" ;	
	AND Claim.admis_date BETWEEN gtStartDate AND gtEndDate ;	
INTO TABLE (gcFundCode+"_H_Claim")
**
*Query Current Month Report
*
SELECT Claim.notify_no, Claim.notify_date, IIF(Claim.service_type = "OPD", "OPD", "IPD") AS serv_type, ;
	Claim.fundcode, Claim.policy_no, Claim.family_no, Claim.client_name, Claim.plan, LEFT(Claim.plan,2) AS plan_type, ;
	Claim.admis_date, Claim.fcharge, Claim.fbenfpaid, Claim.scharge, Claim.sbenfpaid, Claim.fax_by, Claim.inv_page, ;
	IIF(EMPTY(Claim.fax_by), Claim.abenfpaid, Claim.exgratia) AS exgratia, ;
	IIF(EMPTY(Claim.fax_by), Claim.sbenfpaid, Claim.fbenfpaid) AS paid, ;
	LEFT(result,1) AS status, Claim.result, Claim.return_date, Claim.unclean, Claim.unclean_note, ;
	((return_date-TTOD(notify_date)) - holidays(TTOD(notify_date), return_date)) AS workdays ;	
...
INTO TABLE (gcFundCode+"_H_Claim_Month")
***
...
FROM (gcFundCode+"_H_Claim_Month") A INNER JOIN cims!claim_line ;
	ON  A.notify_no = Claim_line.notify_no ;
WHERE A.serv_type = "IPD" ;
	AND A.status = "P" ;	
INTO TABLE (gcFundCode+"_H_Claimline")
*
...
FROM (gcFundCode+"_H_Claim_Month") ;
GROUP BY 1 ;
WHERE result = "P1" ;
	AND (result == "D" OR  len(alltrim(result)) = 2) ;
	AND EMPTY(unclean) ;
INTO TABLE (gcFundCode+"_H_Aging")
*
**********************************
...
ORDER BY 1 ;		
INTO TABLE (gcFundCode+"_H_PlanbyService_Month")
...
FROM _SumType RIGHT JOIN _SumMember ;
	ON _SumType.plan = _SumMember.plan ;
ORDER BY 1 ;		
INTO TABLE (gcFundCode+"_H_PlanbyService_Year")
*
**********************************
...
FROM (gcFundCode+"_H_Claimline") ;
GROUP BY plan, descriptio, benefit ;
ORDER BY plan ;
INTO TABLE (gcFundCode+"_H_PlanbyCategory_Month")
*
...
lcRollingFile = gcFundCode+"_H_Rolling"
*
...
DO WHILE ldEndDate < gdEndDate
	...
	***********************************************************************
	SELECT YEAR(notify_date) AS yy, MONTH(notify_date) AS mm, SUM(IIF(EMPTY(Claim.fax_by), sbenfpaid, fbenfpaid)) AS paid, ;
		SUM(IIF(EMPTY(Claim.fax_by) AND Claim.service_type <> "OPD", sbenfpaid, fbenfpaid)) AS ipaid, ;
		SUM(IIF(EMPTY(Claim.fax_by) AND Claim.service_type = "OPD", sbenfpaid, fbenfpaid)) AS opaid ;
	FROM cims!claim ;
	WHERE Claim.fundcode = gcFundCode ;
