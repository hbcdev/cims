...
********************
gcCaption = "DVS SCG Monthly Report"
gnAll = 1
gnCover = 1
gnData = 0
gnType = 1
...
FROM Q_memb ;	
INTO TABLE (gcFundCode+"_CTAC_member")	
*
SELECT plan, ;
...
INTO TABLE (gcFundCode+"_CTAC_Claim")
**
...
INTO TABLE (gcFundCode+"_CTAC_Claim_Month")
***
...
FROM (gcFundCode+"_CTAC_Claim_Month") A INNER JOIN cims!claim_line ;
	ON  A.notify_no = Claim_line.notify_no ;
WHERE A.serv_type = "IPD" ;
	AND A.status = "P" ;	
INTO TABLE (gcFundCode+"_CTAC_Claimline")
*
...
FROM (gcFundCode+"_CTAC_Claim_Month") ;
GROUP BY 1 ;
WHERE status $ "DP" ;
	AND EMPTY(unclean) ;
	AND result # "P5" ;
INTO TABLE (gcFundCode+"_CTAC_Aging")
*
**********************************
...
ORDER BY 1 ;		
INTO TABLE (gcFundCode+"_CTAC_PlanbyService_Month")
...
FROM _SumType RIGHT JOIN _SumMember ;
	ON _SumType.plan = _SumMember.plan ;
ORDER BY 1 ;		
INTO TABLE (gcFundCode+"_CTAC_PlanbyService_Year")
*
**********************************
...
FROM (gcFundCode+"_CTAC_Claimline") ;
WHERE cat_code # "OTHER" ;
GROUP BY plan, cat_code, benefit ;
ORDER BY plan ;
INTO TABLE (gcFundCode+"_CTAC_PlanbyCategory_Month")
*
...
lcRollingFile = gcFundCode+"_CTAC_Rolling"
*
...
DO WHILE ldEndDate < gdEndDate
	...
	*
	SELECT SUM(IIF(!EMPTY(start_month) AND !EMPTY(end_month), 1, 0)) AS nom, ;
...
lcRollingFile = gcFundCode+"_CTAC_ClaimRolling"
*
