...
FROM Q_memb ;	
INTO TABLE (gcFundCode+"_DEN_member")	
*
SELECT plan, ;
...
WHERE Claim.fundcode = gcFundCode ;
	AND Claim.claim_with = "D" ;
	AND Claim.result # "C" ;	
	AND Claim.admis_date BETWEEN gtStartDate AND gtEndDate ;	
INTO TABLE (gcFundCode+"_DEN_Claim")
***
...
FROM (gcFundCode+"_DEN_Claim") A INNER JOIN cims!claim_line ;
	ON  A.notify_no = Claim_line.notify_no ;	
	AND A.status = "P" ;	
WHERE claim_line.scharge <> 0 ;	
INTO TABLE (gcFundCode+"_DEN_Claimline_Year")
**
...
INTO TABLE (gcFundCode+"_DEN_Claim_Month")
***
*Query Claim by cause
SELECT A.fundcode, A.notify_no, A.notify_dat, A.cause_type, A.policy_no, A.plan, A.agent, ;
  ...
  IIF(EMPTY(A.fax_by), Claim_line.spaid, Claim_line.fpaid) AS paid, ;
  IIF(EMPTY(A.fax_by), Claim_line.apaid, Claim_line.exgratia) AS exgratia, A.result, A.status ;  
FROM (gcFundCode+"_DEN_Claim_Month") A INNER JOIN cims!claim_line ;
	ON  A.notify_no = Claim_line.notify_no ;
WHERE claim_line.scharge <> 0 ;
INTO TABLE (gcFundCode+"_DEN_Claimline_Month")
*
...
FROM (gcFundCode+"_DEN_Claim_Month") ;
GROUP BY 1 ;
WHERE INLIST(status, "D", "P") ;
	AND EMPTY(unclean) ;
	AND result # "P5" ;
INTO TABLE (gcFundCode+"_DEN_Aging")
*
**********************************************
...
GROUP BY 1 ;
ORDER BY 1 ;
INTO TABLE (gcFundCode+"_DEN_PlanbyItems_Month")
...
SELECT cause_type, ;
	...
	SUM(IIF(status = "W", 1, 0)) AS w_noc, ;
	SUM(IIF(status = "W", paid, 0)) AS w_paid ;
FROM (gcFundCode+"_DEN_Claim") ;
GROUP BY 1 ;
ORDER BY 1 ;
INTO TABLE (gcFundCode+"_DEN_PlanbyItems_Year")
*
**********************************************************
PROCEDURE q_rolling

lcRollingFile = gcFundCode+"_DEN_Rolling"
*
