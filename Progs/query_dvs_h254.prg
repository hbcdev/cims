...
********************
gcCaption = "Monthly Report By Policy Group"
gnAll = 1
gnCover = 1
gnData = 0
gnType = 1
...
IF EMPTY(gcFundCode) AND EMPTY(gdStartDate) AND EMPTY(gdEndDate) AND EMPTY(gcPolicyNo)
	RETURN 
ENDIF
...
lcPolicyNo = "%"+SUBSTR(policy_no, AT("H", policy_no), 8) &&"%H0000254-NHG"
lcPolNo = STRTRAN(SUBSTR(policy_no, AT("H", policy_no), 8), "0", "")+RIGHT(ALLTRIM(policy_no),3)
...
FROM Q_memb ;	
INTO TABLE (gcFundCode+"_"+lcPolNo+"_member")	
*
SELECT plan, ;
...
FROM (gcFundcode+"_"+lcPolNo+"_member") ;
GROUP BY plan ;
ORDER BY plan ;
...
INTO TABLE (gcFundCode+"_"+lcPolNo+"_Claim")
**
...
INTO TABLE (gcFundCode+"_"+lcPolNo+"_Claim_Month")
***
...
FROM (gcFundCode+"_"+lcPolNo+"_Claim_Month") A INNER JOIN cims!claim_line ;
	ON  A.notify_no = Claim_line.notify_no ;
WHERE A.serv_type = "IPD" ;
	AND A.status = "P" ;	
INTO TABLE (gcFundCode+"_"+lcPolNo+"_Claimline")
*
...
FROM (gcFundCode+"_"+lcPolNo+"_Claim_Month") ;
GROUP BY 1 ;
WHERE status $ "DP" ;
	AND EMPTY(unclean) ;
	AND result # "P5" ;
INTO TABLE (gcFundCode+"_"+lcPolNo+"_Aging")
*
**********************************
...
ORDER BY 1 ;		
INTO TABLE (gcFundCode+"_"+lcPolNo+"_PlanbyService_Month")
...
FROM (gcFundCode+"_"+lcPolNo+"_Claim") ;
GROUP BY 1 ;
ORDER BY 1 ;
INTO CURSOR noc
*
...
FROM _SumType RIGHT JOIN _SumMember ;
	ON _SumType.plan = _SumMember.plan ;
ORDER BY 1 ;		
INTO TABLE (gcFundCode+"_"+lcPolNo+"_PlanbyService_Year")
*
**********************************
...
FROM (gcFundCode+"_"+lcPolNo+"_Claimline") ;
WHERE cat_code # "OTHER" ;
GROUP BY plan, cat_code, benefit ;
ORDER BY plan ;
INTO TABLE (gcFundCode+"_"+lcPolNo+"_PlanbyCategory_Month")
*
...
lcRollingFile = gcFundCode+"_"+lcPolNo+"_Rolling"
*
...
DO WHILE ldEndDate < gdEndDate
	...
	*
	SELECT SUM(IIF(!EMPTY(start_date) AND !EMPTY(end_month), 1, 0)) AS nom, ;
	...
	*
	SELECT notify_no, claim_date, IIF(service_type = "IPD", "IPD", "OPD")  AS service, ;
	IIF(EMPTY(claim.fax_by), sbenfpaid, fbenfpaid) AS paid, result ;
	FROM cims!claim ;
	WHERE Claim.fundcode = gcFundCode ;
	AND Claim.policy_no LIKE lcPolicyNo ;	
	AND Claim.claim_with = "T" ;
...
lcRollingFile = gcFundCode+"_"+lcPolNo+"_ClaimRolling"
*
