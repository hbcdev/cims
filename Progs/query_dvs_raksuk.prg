...
FROM Q_memb ;	
INTO TABLE (gcFundCode+"_R_member")	
*
SELECT plan, ;
...
INTO TABLE (gcFundCode+"_R_Claim")
**
...
INTO TABLE (gcFundCode+"_R_Claim_Month")
***
...
FROM (gcFundCode+"_R_Claim_Month") A INNER JOIN cims!claim_line ;
	ON  A.notify_no = Claim_line.notify_no ;
WHERE A.serv_type = "IPD" ;
	AND A.status = "P" ;	
INTO TABLE (gcFundCode+"_R_Claimline")
*
...
FROM (gcFundCode+"_R_Claim_Month") ;
GROUP BY 1 ;
WHERE result = "P1" ;
	AND (result == "D" OR  len(alltrim(result)) = 2) ;
	AND EMPTY(unclean) ;
INTO TABLE (gcFundCode+"_R_Aging")
*
**********************************
...
ORDER BY 1 ;		
INTO TABLE (gcFundCode+"_R_PlanbyService_Month")
...
FROM _SumType RIGHT JOIN _SumMember ;
	ON _SumType.plan = _SumMember.plan ;
ORDER BY 1 ;		
INTO TABLE (gcFundCode+"_R_PlanbyService_Year")
*
**********************************
...
FROM (gcFundCode+"_R_Claimline") ;
GROUP BY plan, descriptio, benefit ;
ORDER BY plan ;
INTO TABLE (gcFundCode+"_R_PlanbyCategory_Month")
*
...
lcRollingFile = gcFundCode+"_R_Rolling"
*
